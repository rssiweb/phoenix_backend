...
MARKSHEET_BODY_STRING = (
    "Please find the marksheets for {} generated on {} in attachments."
)
...
sub_in_template_order = (
    "english maths generalscience drawing_handwriting computer".split()
)
...
def exam_to_dict(examid, att_start, att_end):
    ...
    for std in students:
        ...
        tests = [
            ...
            if test.cat_sub_association.category == std.category
        ]
        marks = []
        for test in tests:
            sub_data = {}
        ...
        # Attendace data
        std_att = Attendance.query.filter(
            Attendance.date >= att_start,
            Attendance.date <= att_end,
...
def build_marksheets(meta, examid, att_start, att_end):
    ...
    marksheet_filenames = []
    for std in students:
        header, row, rowtotal, footer = map(
            Image.open,
            [
                "app/static/marksheet_header.jpg",
