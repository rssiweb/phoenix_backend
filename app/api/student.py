...
def list_all_by_endDate(endDate):
    ...
    students = Student.query.filter(
        or_(Student.effective_end_date == None, Student.effective_end_date >= endDate)
    )
...
def list_by_branch(branchid):
    students = Student.query.filter(
        Student.branch_id == branchid, Student.isActive != False
    )
    students = [s.serialize() for s in students]
    faculty = Faculty.query.get(request.user.id)
    if not all([faculty, faculty.admin]):
...
def by_ids():
    ...
    if ids:
        students = [
            s.serialize() for s in Student.query.filter(Student.id.in_(ids)).all()
        ]
    faculty = Faculty.query.get(request.user.id)
    if not all([faculty, faculty.admin]):
        for s in students:
