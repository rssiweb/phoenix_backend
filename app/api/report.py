...
def enqueue_attendance():
    ...
    message = "Attendace report queued, you will receive an email on {} email address.".format(
        request.user.email
    )
...
def enqueue_exam(examid):
    ...
    message = "Exam report queued, you will receive an email on {} email address.".format(
        request.user.email
    )
    return jsonify(dict(status="success", message=message))
...
def enqueue_marksheet(examid):
    ...
    message = "Marksheet report queued, you will receive an email on {} email address.".format(
        request.user.email
    )
    return jsonify(dict(status="success", message=message))
...
def get_branch(branchid):
    ...
    message = "I-Card generation is queued, you will receive an email on {} email address.".format(
        request.user.email
    )
    return jsonify(dict(status="success", message=message))
