...
api = Blueprint('admin_dist_type_api', __name__,
                url_prefix='/api/admin/dist_type')
...
def add():
    ...
    query = DistributionType.query.filter(func.lower(
        DistributionType.name) == func.lower(name))
    if query.count() > 0:
