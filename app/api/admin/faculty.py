...
def update_faculty():
    ...
    if missing_required_fields:
        res["message"] = "expected fields {0} not found".format(missing_required_fields)
        return jsonify(res), res_code
    ...
    blank_values = [key for key in required_fields if not data.get(key, "").strip()]
    if blank_values:
        res["message"] = "values for field(s) {0} is required".format(blank_values)
        return jsonify(res), res_code
...
def set_faculty_admin(facid, admin):
    ...
    return jsonify(
        dict(
            status="success",
            message="{} is {} Admin".format(
                fac.name, "now an" if fac.admin else "no longer an"
            ),
            admin=fac.admin,
        )
    )
