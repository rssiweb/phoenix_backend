...
def set_punch_in(attendance, date, inTime, studentid=None):
    ...
    if inTime == "" and attendance:
        ...
        if not isValid:
            return (
                jsonify(
                    dict(status="fail", message="time is not valid {}".format(inTime))
                ),
                200,
            )
        if attendance:
    ...
    db.session.commit()
    return (
        jsonify(dict(status="success", message=msg, attendance=attendance.serialize())),
        200,
    )
...
def set_punch_out(attendance, date, outTime):
    ...
    if outTime != "":
        ...
        if not isValid:
            return (
                jsonify(
                    dict(status="fail", message="{} is not valid time".format(outTime))
                ),
                200,
            )
