...
class DistributionItem(Base):
    name = db.Column(db.String(50), nullable=False, unique=True)
    item_type_id = db.Column(db.Integer, db.ForeignKey('distribution_item_type.id'), nullable=False)
...
class Distribution(Base):    
    ...
    distribution_item_id = db.Column(db.Integer, db.ForeignKey('distribution_item.id'), nullable=False)
    distribution_item = relationship("DistributionItem", back_populates="distributions")

    distribution_type_id = db.Column(db.Integer, db.ForeignKey('distribution_type.id'), nullable=False)
